...
def job_config = [
        job: [
                name: "asdasdasd22_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asdasdasd22_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asdasdasd22_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asdasdasd22.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asdasdasd22", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asdasdasd22_Pipeline_develop");

job.with {

    authenticationToken('K8mB1Xs6Zs9qWZz6heoVvFiaoghA0chq')
